...
当前 revision：r23（r22 T301b done；r23 T301c done——取消/超时/消息上限/注入遏制/会话隐私契约化）。v0.1 必需任务 10/10 done；T011 done；T101 in_progress（VS Code 交互待重设计）；T201 in_progress（挂起：V03-2 评估债 + JetBrains 壳环境阻塞）；T301 in_progress（T301a/b/c done，T301d ready——待 Codex 网络恢复）。拆分任务须保留原 ID、依赖和 AC 追踪。
...
- 当前目标：T301d 双宿主验收（T301a/b/c 已收口：E027—E030）。
...
- 阻塞：Codex CLI 上游代理 502（CC Switch 本地代理→127.0.0.1:15721 失败），第二宿主闭环实测待网络恢复（T301d 唯一硬阻塞）；Claude Code 侧已批准可用（E028）。
- 下一步：T301d——Claude Code 宿主内 review 双轮闭环实测；Codex 网络恢复后同流程；README/SUPPORT Agent Surface 文档；Stop hook opt-in；V04-1..4 验收映射 + 独立 Checker。
- decisions：D011、D012；evidence：E027、E028、E029、E030；最后 revision：r23。
